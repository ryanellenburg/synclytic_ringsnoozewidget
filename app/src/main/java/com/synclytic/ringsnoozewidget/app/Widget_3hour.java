...
public class Widget_3hour extends MainApplication {

    @Override
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        Log.d("Widget_3hour", "(onUpdate) onUpdate called for widget IDs: " + Arrays.toString(appWidgetIds));
        final int snoozeTime = 3; // 3 hours

        for (int appWidgetId : appWidgetIds) {
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.ring_snooze_widget);

            Log.d("Widget_3hour", "(onUpdate) Creating PendingIntent for widgetId: " + appWidgetId);
            ...
            Log.d("Widget_3hour", "(onUpdate) Set click listener on widgetIcon for widgetId: " + appWidgetId);
