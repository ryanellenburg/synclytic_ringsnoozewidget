...
public class Widget_1hour extends MainApplication {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        Log.d("Widget_1hour", "(onUpdate) onUpdate called for widget IDs: " + Arrays.toString(appWidgetIds));
        final int snoozeTime = 1; // 1 hour

        for (int appWidgetId : appWidgetIds) {
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.ring_snooze_widget);

            Log.d("Widget_1hour", "(onUpdate) Creating PendingIntent for widgetId: " + appWidgetId);
            ...
            Log.d("Widget_1hour", "(onUpdate) Set click listener on widgetIcon for widgetId: " + appWidgetId);
