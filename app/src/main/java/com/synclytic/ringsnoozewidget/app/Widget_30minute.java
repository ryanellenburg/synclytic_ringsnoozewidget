...
public class Widget_30minute extends MainApplication {

    @Override
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        Log.d("Widget_30minute", "(onUpdate) onUpdate called for widget IDs: " + Arrays.toString(appWidgetIds));
        final int snoozeTime = 30; // 30 minutes

        for (int appWidgetId : appWidgetIds) {
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.ring_snooze_widget);

            Log.d("Widget_30minute", "(onUpdate) Creating PendingIntent for widgetId: " + appWidgetId);
            ...
            Log.d("Widget_30minute", "(onUpdate) Set click listener on widgetIcon for widgetId: " + appWidgetId);
